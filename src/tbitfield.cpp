...
{
    if (len < 0) throw std::invalid_argument("Field length should be positive integer");
...
{
    if ((n < 0) || (n > BitLen)) throw std::invalid_argument("Incorrect n value Index");
    return n / (8 * sizeof(TELEM));
}

TELEM TBitField::GetMemMask(const int n) const // битовая маска для бита n
{
    if ((n < 0) || (n > BitLen)) throw std::invalid_argument("Incorrect n value Mask");
...
{
    if ((n < 0) || (n > BitLen)) throw std::invalid_argument("Incorrect n value set");
    pMem[GetMemIndex(n)] |= GetMemMask(n);
}

void TBitField::ClrBit(const int n) // очистить бит
{
    if ((n < 0) || (n > BitLen)) throw std::invalid_argument("Incorrect n value clr");
    pMem[GetMemIndex(n)] &= ~GetMemMask(n);
...
{
    if ((n < 0) || (n > BitLen)) throw std::invalid_argument("Incorrect n value get");
